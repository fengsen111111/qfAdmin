...
import {APPConfig} from "../../config"; // 字数统计插件
...
onBeforeMount(() => {
  if (props.structure.value)
    if (props.structure.value.includes(APPConfig.apiURL)) {
      global.axios.download(null, props.structure.value, {name: ""}, global, "rich_text").then(res => {
        component_state.myValue = res
      });
      console.log(component_state.myValue)
    } else {
      let xhr = new XMLHttpRequest();
      let okStatus = document.location.protocol === "file:" ? 0 : 200;
      xhr.open("GET", props.structure.value, false);
