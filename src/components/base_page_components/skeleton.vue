  ...
  <a-layout>
    <a-layout-sider class="tab-menu">
      ...
      <div class="tab-menu-list">
        <div
            v-for="item in skeleton_state.menuData"
            :key="item.value"
            align="center"
            class="tab-menu-item"
        >
          <div
              :class="item.checked ? 'tab-menu-card-checked' : 'tab-menu-card'"
              :style="{ color: item.font_color }"
              @click="showFirstMenu(item.value)"
              @mouseleave="changeFontColor(item.value, 'out')"
    ...
    <a-layout-sider v-show="!skeleton_state.collapsed" class="menu">
      ...
      <div class="menu-list">
        <a-menu
            v-model:selectedKeys="skeleton_state.menuCheckedValues"
            mode="inline"
        >
          <template
              v-for="(item, index) in skeleton_state.tabCheckedMenu"
              :key="index"
          >
            <a-sub-menu
    ...
    <a-layout>
      <a-layout-header>
        <div
            class="icon-font"
            style="color: #656565 !important; font-size: 22px"
        >
          <span
              v-if="!skeleton_state.collapsed"
              class="iconfont"
              @click="skeleton_state.collapsed = !skeleton_state.collapsed"
          >&#xe79a;</span
          >
          <span
              v-else
        ...
        <div class="tag">
          <a-tag
              v-for="tag in skeleton_state.tags"
              :key="tag.value"
              :class="tag.checked_status ? 'active-tag' : ''"
              :closable="tag.value === -1 ? false : true"
              @click="openTag(tag)"
              @close="closeTag(tag.page_key)"
          >
            {{ tag.label }}
          </a-tag>
        </div>
        <div style="height: 60px; width: 50px">
          <a-tag
              v-if="skeleton_state.tags.length > 1"
              style="
              float: right;
              line-height: 28px;
              height: 28px;
            ...
            "
              @click="closeAllTag()"
          ><span class="iconfont" style="font-size: 26px"
          >&#xe6a1;</span
          ></a-tag
          >
        </div>
        ...
        <a-dropdown>
          <a
              v-show="global.languageList.length > 1 ? true : false"
              class="ant-dropdown-link"
              href="javascript:;"
          >
            <div
                class="language"
                style="margin-right: 20px; color: #515a6e !important"
            >
              {{ global.language.name }}
              <span
                  class="iconfont"
                  style="font-size: 22px; float: right; margin-left: 5px"
              >&#xe64d;</span
              >
            </div>
          </a>
          <template #overlay>
            <a-menu>
              <a-menu-item
                  v-for="(item, index) in global.languageList"
                  :key="index"
      ...
      <a-layout-content>
        <div
            v-for="page in skeleton_state.openedPages"
            :key="page.page_key"
            v-show="page.open_status"
            class="content"
        >
          <keep-alive>
            <component
                :is="allPageComponents[page.type]"
                :pageData="page"
...
<!--  全局视频组件-->
  <a-modal v-model:visible="global.videoState.isCollapse" :centered="true" :closable="true" :footer="null"
           :keyboard="false"
           @cancel="closeVideo"
           :maskClosable="false" :width="580+'px'" :height="280+'px'">
    <vue3VideoPlay
        v-bind="global.videoState.options"
    />
  </a-modal>
