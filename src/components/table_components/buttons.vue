...
<template>
        ...
        <a-menu>
                  <span v-for="(button,buttonIndex) in props.structure.config">
                        ...
                        <a-button
                            ...
                            @click="handle(button)">
                            <span v-if="button.showValue"
                                  :class="button.buttonType?button.buttonType:'primary'">（ {{
  ...
  <div v-else >
    <a-button
        v-for="(button,buttonIndex) in props.structure.config"
        v-show="showButton(button)"
        :key="buttonIndex"
        :type="button.buttonStyle?button.buttonStyle:'link'"
        @click="handle(button)">
      <span v-if="button.showValue"
            :class="button.buttonType?button.buttonType:'primary'">{{
          global.findLanguage(rowValue[button.showValue])
