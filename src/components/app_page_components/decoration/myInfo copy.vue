<script setup>
	...
	function _shopInfo() {
		...
		global.axios.post('decoration/Store/webGetStoreInfo', {
			...
			.then(res => {
				...
				setTimeout(() => {
					formzdcz.value.auto_power_line = shopObj.value.auto_power_line
					formzdcz.value.one_power_money = shopObj.value.one_power_money
					formzdcz.value.day_max_power_money = shopObj.value.day_max_power_money
				}, 500);
			})
	}
	...
	const screenWidth = ref(window.innerWidth); //屏幕尺寸
	onMounted(() => {
		window.addEventListener("resize", () => {
...
<template>
	...
	<div v-if="shopObj.open_h_store_account">
		...
		<div v-if="(shType=='b'&&shopObj.open_h_store_account!='a')||is_ptsj == '平台'">
			<!--搜索-->
			<div style="padding: 10px 20px;overflow: auto;max-height: 88vh;">
				<div class="a16">
					<div class="a17">
					...
					<div class="a21">
						...
						<div class="a22C" @click="handleExport">导出详情</div>
						...
						<div class="a25">
							<a-row>
								<template v-for="item in infoArr" :key="item">
									<a-col :lg="8" :xl="6" :sm="12">
										<div class="a27">
											<div class="a26">{{item.name}}：</div>
								...
								<a-col :lg="8" :xl="6" v-if="shopObj.store_type!='a'">
									<div  class="a33">
								...
								<a-col :lg="8" :xl="6" v-if="shopObj.type=='a'">
									<div  class="a33">
				...
				<div v-if="titleType=='店铺数据'">
					...
					<div style="display: flex;width: 100%;">
						<a-row>
							<a-col :xxl="20" :xl="24" :lg="24" style="border-right: 10px solid #f5f5f5;">
								<div>
									<div class="a23">基础数据</div>
									<!-- <div class="a42"></div> -->
									<a-row>
										<a-col :xl="6" :lg="8" class="col-box">
											<div class="a44" @click="rqtz('购物商城','商品管理')">
												<div class="a45">
													<img src="../../../../public/resource/image/home/home1.png"
														style="width: 24px;height: 24px;position: relative;top:0px;"
														alt="">
												</div>
												<div>
													<div>商品总数</div>
													<div class="a47">{{Number(shopObj.goods_number).toLocaleString()}}
													</div>
												</div>
											</div>
										</a-col>
										<a-col :xl="6" :lg="8" class="col-box">
											<div class="a44">
												...
												<div>
													<div>今日营业额</div>
													<div class="a47">{{Number(shopObj.today_money).toLocaleString()}}
													</div>
												</div>
											</div>
										</a-col>
										<a-col :xl="6" :lg="8" class="col-box">
											<div class="a44">
												...
												<div>
													<div>本月营业额</div>
													<div class="a47">{{Number(shopObj.month_money).toLocaleString()}}
													</div>
												</div>
											</div>
										</a-col>
												...
												<div>
													...
													<div style="display: flex;align-items: center;">
														...
														<div style="margin-left: 10px;">
															<div>
																商家保证金：{{Number(shopObj.pay_deposit_money).toLocaleString()}}
															</div>
															<div>
																分类保证金：{{Number(shopObj.payed_type_prices).toLocaleString()}}
															</div>
														</div>
													</div>
												</div>
										...
										<a-col :xl="6" :lg="8" class="col-box">
											<div class="a44" @click="rqtz('购物商城','订单管理')">
												...
												<div>
													<div>总订单数</div>
													<div class="a47">
														{{Number(shopObj.all_order_number).toLocaleString()}}</div>
												</div>
											</div>
										</a-col>
										<a-col :xl="6" :lg="8" class="col-box">
											<div class="a44" @click="rqtz('购物商城','订单管理')">
												...
												<div>
													<div>今日订单数</div>
													<div class="a47">
														{{Number(shopObj.today_order_number).toLocaleString()}}
													</div>
												</div>
											</div>
										</a-col>
										<a-col :xl="6" :lg="8" class="col-box">
											<div class="a44" @click="rqtz('购物商城','订单管理')">
												...
												<div>
													<div>本月订单数</div>
													<div class="a47">
														{{Number(shopObj.month_order_number).toLocaleString()}}
													</div>
												</div>
											</div>
										</a-col>
										<a-col :xl="6" :lg="8" class="col-box">
											<div class="a52">
												...
												<div>
													<div>曝光量余额</div>
													<div style="display: flex;align-items: center;">
														<div class="a47">{{Number(shopObj.power).toLocaleString()}}
														</div>
														<a-button v-if="is_ptsj!='平台'" @click.stop="titleType='资金日志'"
															style="margin-left: 5px;">去充值</a-button>
														<a-button @click="zdcz_vis=true"
															style="margin-left: 5px;">曝光设置</a-button>
														<!-- <div><span @click="zdcz_vis=true"
																style="cursor: pointer; color: #0c96f1;">曝光设置</span></div> -->
													</div>
													<!-- <div>自动充值曝光量：<span @click="zdcz_vis=true"
															style="cursor: pointer; color: #0c96f1;">重新设置</span></div>
													<div>低于<text
															style="color: #FF0000;">{{shopObj.auto_power_line}}</text>曝光量时自动充值
													</div>
													<div>单次自动充值金额<text
															style="color: #FF0000;">{{shopObj.one_power_money}}</text>
													</div>
													<div>每天自动最大充值金额 <text
								...
								<div>
									<div class="a58">
										<div class="a59">
											...
											<div class="a60"
												@click="()=>{lookAll = true;allTitle='商品销量排行';allShopList()}">查看更多
											</div>
										</div>
										<div class="a61" v-for="(item,index) in spxlphlist" :key="item.id">
											<div style="display: flex;">
												...
												<div style="margin-left: 20px;">
													...
													{{ screenWidth >= 1200 ? (item.name.length > 6 ?
													item.name.slice(0,6) + '...' : item.name) : item.name }}
												</div>
											</div>
									...
									<div class="a58">
										<div class="a59">
											...
											<div class="a60"
												@click="()=>{lookAll = true;allTitle='投流商品排行';allShopList()}">查看更多
											</div>
										</div>
										<div class="a61" v-for="item in sptlphlist" :key="item.id">
											<div v-if="screenWidth >= 1200" style="width: 130px;">
												{{item.name.length>6?item.name.slice(0,6)+'...':item.name}}
											</div>
				...
				<div v-if="titleType=='资金日志'">
					...
					<!-- <div class="a80" style="align-items: center;width: 100%;border: 1px solid red;"> -->
						<a-row>
							<a-col :xl="24" :xxl="2">
								<div style="color: #000000;font-size: 18px;"><b>资金流水</b></div>
							</a-col>
							<a-col :xl="12" :xxl="5">
								<div class="a81">
									<div>订单编号</div>
							...
							<a-col :xl="12" :xxl="5">
								<div class="a84" style="cursor: pointer;">
									...
									<div class="a85" @click="exportToExcel()">
