<script setup>
  ...
  console.log('数据',props.structure.warning);
  
  const component_state = reactive({
    isCollapse: false,
    myValue: props.structure.warning,
    init: {
      promotion: false,
  ...
  watch(
    ...
    (newValue) => {
      props.structure.warning = newValue
    }
  )
...
<template>
  <div>
    <a-button class="button-class button-warning" style="margin-left: 0;margin-right: 10px" type="primary" @click="clear">{{ global.findLanguage('清空内容') }} </a-button>
