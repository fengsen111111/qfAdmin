<script setup>
      ...
      .then((res) => {
        ...
        if (res.tableButtons.length > 0) {
          ...
          table_state.tableButtons.forEach((button, index) => {
            ...
            // 批量发送
            else if (button.handleType == "sendMoreGoods") {
              table_state.rowSelection = { onChange: onSelectChange };
            }
            // 批量修改库存 
            else if (button.handleType == "setMoreStock") {
              table_state.rowSelection = { onChange: onSelectChange };
            }
          });
        } else {
  ...
  let visiblePL = ref(false)//批量发送
  let visibleKC = ref(false)//批量库存
  let kcVal = ref(1)//库存
  let mdVal = ref([])//接收门店
  let mdData = ref([])//门店数据
  let clickData = ref({})//当前点击数据，接口等

  // 打开批量发送弹框
  function handAll() {
    if (table_state.selectKeys.length > 0) {
      visiblePL.value = true
    } else {
  ...
  // 批量发送弹框确定
  function handleOk() {
    if (mdVal.value.length > 0) {
      const post_params = {
        ids: table_state.selectKeys,//商品ID数组
        companys: mdVal.value//接收公司ID，数组
      }
      // console.log('post_params', post_params);
      global.axios.post(clickData.value.requestUrl, post_params, global, false, true)
        .then((res) => {
          message.success('操作成功');
          visiblePL.value = false
          setTimeout(() => {
            refresh()
          }, 2000);
        })
  ...
  // 批量设置库存弹框确定
  function handleKcOk() {
    console.log('批量发送弹框确定');
    if (kcVal.value > 0) {
      const post_params = {
        ids: table_state.selectKeys,//商品ID数组
        stock: kcVal.value//库存
      }
      global.axios.post(clickData.value.requestUrl, post_params, global, false, true)
        .then((res) => {
          message.success('操作成功');
          visibleKC.value = false
          setTimeout(() => {
            refresh()
          }, 2000);
        })
    } else {
      message.error('商品数不得小于0');
    }
  }
  // =======================================================================
  ...
  function tableHandles(tableHandlesData) {
    ...
    //批量发送
    if (handleInfo.handleType == "sendMoreGoods") {
      mdData.value = handleInfo.companys
      clickData.value = handleInfo
      console.log('批量发送', mdData.value, handleInfo, table_state.selectKeys);
      handAll()
    }
    // 批量修改库存
    if(handleInfo.handleType == "setMoreStock"){
      clickData.value = handleInfo
      console.log('批量修改库存');
      handKC()
    }
...
<template>
  ...
  <!-- 弹框 -->
  <a-modal v-model:visible="visiblePL" title="批量发送" @ok="handleOk">
    <div>
      <div>
        <label>选择接收商品的分公司</label>
        <a-select v-model:value="mdVal" mode="tags" style="width: 100%;margin-top: 10px;" placeholder="请选择接收商品的分公司"
          :options="mdData"></a-select>
      </div>
    </div>
  </a-modal>
  <a-modal v-model:visible="visibleKC" title="批量设置库存" @ok="handleKcOk">
    <div>
      <div>
        <label>输入库存</label>
        <a-input v-model:value="kcVal" onkeyup="value=value.replace(/[^\d]/g,'')" style="margin-top: 10px;"
          placeholder="请输入库存" />
      </div>
    </div>
  </a-modal>
